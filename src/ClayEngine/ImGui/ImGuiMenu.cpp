...
namespace Clay
{
   ...
   void ImGuiMenu::MeshPrimitiveOptions(std::vector<Ref<Model>>& _models)
   {
      ...
      {
         modelLines = std::make_shared<LineMesh>(glm::vec4(0.6, 0.4, 0.3, 1.0), nullptr);
         MeshTools::BoundingBox(modelLines, 0.1, 0.2, 0.2);
//         modelLines->Print();
      }
      ...
      if (modelCloud != nullptr)
         _models.push_back(std::move(std::dynamic_pointer_cast<Model>(modelCloud)));
      if (modelLines != nullptr)
         _models.push_back(std::move(std::dynamic_pointer_cast<Model>(modelLines)));
      ImGui::End();
   }
   ...
   void ImGuiMenu::RendererOptions()
   {
      ...
      ImGui::Text("Line Count: %d", lineStats.GetTotalLineCount());
