...
namespace Clay
{
   LineMesh::LineMesh(const glm::vec4& color, const Ref<Model>& parent) : Model(parent, true, 400000)
   {
      _mesh->_color = color;
      _mesh->_type = RendererAPI::MODE::Lines;
   }

   void LineMesh::InsertVertex(float x, float y, float z)
   {
...
//         CLAY_LOG_INFO("Inserting: {} {} {}", x, y, z);
   ...
   void LineMesh::InsertIndex(uint32_t index)
   {
   ...
   void LineMesh::Print()
   {
      ...
      for(int i = 0; i< _mesh->_indices.size(); i++)
      {
         CLAY_LOG_INFO("Index: {}", _mesh->_indices[i]);
      }
   }
