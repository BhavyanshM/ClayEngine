...
namespace Clay
{
   ...
   void MeshTools::Sphere(Ref<TriangleMesh>& model, float radius, int sectors, int stacks, const SurfaceParameters& params, float dt)
   {
      ...
      for (uint16_t i = 0; i <= stacks; i++)
      {
         ...
         for (uint16_t j = 0; j <= sectors; j++)
         {
            ...
            h = params.CalculateHeight(u * 80, v * 80, dt) * 0.02f;
   ...
   void MeshTools::BoundingBox(Ref<LineMesh>& model, float height, float width, float length)
   {
      float v[8][3] = {height / 2, width / 2, length / 2, height / 2, -width / 2, length / 2, -height / 2, -width / 2, length / 2, -height / 2, width / 2,
                       length / 2, height / 2, width / 2, -length / 2, height / 2, -width / 2, -length / 2, -height / 2, -width / 2, -length / 2, -height / 2,
                       width / 2, -length / 2};

      uint8_t lines = 4;
      for (int i = 0; i < lines; i++)
      {
         model->InsertVertex(v[i][0], v[i][1], v[i][2]);
         model->InsertVertex(v[(i + 1) % lines][0], v[(i + 1) % lines][1], v[(i + 1) % lines][2]);
      }
      for (int i = 0; i < lines; i++)
      {
         model->InsertVertex(v[i + lines][0], v[i + lines][1], v[i + lines][2]);
         model->InsertVertex(v[(i + 1) % lines + lines][0], v[(i + 1) % lines + lines][1], v[(i + 1) % lines + lines][2]);
      }
      for (int i = 0; i < lines; i++)
      {
         model->InsertVertex(v[i][0], v[i][1], v[i][2]);
      ...
      //      // Set up Index Buffer for both top-bottom and side edges.
      //      uint8_t lines = 4;
      //      int offset = 0;
      //      for (uint8_t i = 0; i < lines; i++)
      //      {
      //         model->InsertIndex(offset);
      //         model->InsertIndex((offset + 1) % lines);
      //         offset++;
      //      }
      //      for (uint8_t i = 0; i < lines; i++)
      //      {
      //         model->InsertIndex(offset);
      //         model->InsertIndex((offset + 1) % lines * 2);
      //         offset++;
      //      }
      //      offset = 0;
      //      for (uint8_t i = 0; i < lines; i++)
      //      {
      //         model->InsertIndex(offset);
      //         model->InsertIndex(offset + lines);
      //         offset++;
      //      }
   }
