...
namespace Clay
{
   OpenCL::OpenCL(const std::string& packagePath)
   {
   ...
   uint8_t OpenCL::CreateLoadReadOnlyImage2D_RGBA8(uint8_t *colorBuffer, uint32_t width, uint32_t height)
   {
      images.emplace_back(
   ...
   void OpenCL::SetArgument(const std::string& kernel, uint8_t argId, uint8_t bufferId, bool image)
   {
      (image) ? kernels[kernel].setArg(argId, images[bufferId]) : kernels[kernel].setArg(argId, buffers[bufferId]);
   }

   void OpenCL::SetArgumentInt(const std::string& kernel, uint8_t argId, uint32_t value)
   {
